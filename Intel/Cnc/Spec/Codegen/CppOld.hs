     ...
     maybeComment [t"", t"", t"Next we define 'private contexts'.",
	  ...
	  (do 
	   ...
	   maybeComment [t"",t"Next, collection/thread local state used/generated by plugins:"] $
	      forM_ plugins $ \pg -> fst$ addLocalState pg stp
	   ...
	   forM_ (AM.toList tags) $ \ (tg, Just ty) -> 
	     --var (TRef TTemplate "CnC::tag_collection" ty) (atomToSyn tg)
	     putD$ text "CnC::tag_collection" <> angles (cppType ty) <> t" & " <> textAtom tg <> semi
	   ...
	   -- FIXME: No reason to wrap ALL item collections.
	    ...
	    cppClass (Syn wrapper) (s"")
  	   ...
  	   comm ""
 	   comm "Item collections are all wrapped for now:" 
	   forM_ (AM.toList items) $ \ (itC, Just (ty1,ty2)) -> 
