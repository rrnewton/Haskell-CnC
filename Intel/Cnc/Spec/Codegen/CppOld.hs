...
maybeCommentDoc ls txt = 
  if txt == t""
  then txt
  else vcat (map (\ comm -> if comm == t"" then comm else t"// "<> comm) ls) $$ txt

maybeComment ls m = 
  let doc = execEasyEmit m in
  if doc == t""
  then return ()
...
-- because I am IN THE MIDDLE OF switching to use EasyEmit functionality.
   ...
   putD$ cppclass (maincontext <> t " : public CnC::context" <> angles (pad maincontext)) $ 
     ...
     [maybeCommentDoc [t"",t"Next, global state used/generated by plugins:"]
        (execEasyEmit$ 
	 forM_ (toAtom "env" : stepls) $ \stp -> 
	 forM_ plugins $ \pg -> fst$ addGlobalState pg stp)] ++
...
-- public CnC::default_tuner< int, tagfun_depends_context > // TODO3: derive from default tuner
	...
	putD$ 
	  ...
	  (--t "\n  private:" $$
	   ...
	   --maincontext <> t" & m_parent;"  $$ 
           ...
           t"\n  public:" $$
	   t"// First, state for the private context.  Note one private context object is instantiated for EACH thread."$$
	   t"  const "<> (cppType$ TPtr stpty) <+> t"tag" <> semi <> t" // Cache the tag for each step instance." $$

           -- Execute plugin hooks:
	   maybeCommentDoc [t"",t"Next, collection/thread local state used/generated by plugins:"]
		           (execEasyEmit$ forM_ plugins $ \pg -> fst$ addLocalState pg stp) $$
	   ...
	   -- Wrap tag collections:
           ------------------------------------------------------------   
	   t "// Tag collections are simply aliases to the parents' (NO WRAPPING YET):" $$ 
	   ...
	   (vcat $ 
		...
		--  but it has too many arguments and is poorly abstracted.)
	        wrapGP doret retty nm args isPut = 
			   ...
			   doplugs project = execEasyEmit$ forM_ plugins $ \pg -> 
					       project pg (spec, Syn$ t$ snd$ head args, Syn$ t$ snd$ head$tail args, stp,itC)
	   ...
	   (vcat $ 
	    (flip map) (AM.toList items) $ \ (itC,Just (ty1,ty2)) -> 
	    textAtom itC <> t"_wrapper" <+> textAtom itC <> semi 
	   ) $$
	   ...
	   hangbraces (privcontext stp <> parens (maincontext <> t" & p") <+> colon $$ 
		       (nest 6 $ vcat $ 
			(flip map) (AM.toList items) $ \ (itC, Just (ty1,ty2)) -> 
			textAtom itC <> parens (t"p, this") <> commspc ) $$
		      ...
		      (maybeCommentDoc [t"",t"Next, collection/thread-local state is initalized:"]
		       (execEasyEmit$ forM_ plugins $ \pg -> snd$ addLocalState pg stp))
   ...
   maybeComment [t"", t"", t"Execute method wrappers for each step that uses a private context:"] $
	...
	(execEasyEmit$ forM_ plugins $ \pg -> beforeStepExecute pg (spec,Syn$ t"ptr", Syn$t"c",stp) (Syn$ t"tag", Syn$ t"ptr", Syn$t"c")) $$
	t"int result = " <+> U.app "m_step.execute" ["tag", "*ptr"] <> semi $$
	(execEasyEmit$ forM_ plugins $ \pg -> afterStepExecute pg (spec,Syn$ t"ptr", Syn$t"c",stp) (Syn$ t"tag", Syn$ t"ptr", Syn$t"c")) $$
	t"return result;"
